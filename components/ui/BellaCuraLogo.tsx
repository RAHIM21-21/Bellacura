...
export default function BellaCuraLogo({ size = 'md' }: { size?: 'sm' | 'md' | 'lg' }) {
  ...
  return (
    <Link href="/" className="flex flex-col items-center leading-none group">
      <span
        className={`${pinyonScript.className} ${sizes[size].script} text-rose-700 group-hover:text-rose-800 transition-colors`}
        style={{ lineHeight: 1 }}
      >
        BellaCura
      </span>
      <span
        className={`${sizes[size].tagline} tracking-[0.25em] uppercase text-rose-400 group-hover:text-rose-500 transition-colors mt-1`}
      >
